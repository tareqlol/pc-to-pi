...
function createWindow () {
	...
	// passing settings to the settings page
	ipcMain.on('get-settings', function (event, keys) {
		keys.forEach(function (key) {
			mainWindow.webContents.send('settings-data', [key, store.get(key)])
		})
	})

	// saving settings to the store
	ipcMain.on('save-settings', function (event, data) {
		Object.keys(data).forEach(function (key) {
			// set the data on the store
			store.set(key, data[key])
			// set the data on process.env 
		...
		submenu: [
			{
				...
				click: function() {
					mainWindow.loadURL('http://localhost:42100/')
				}
			},
			{
				label: 'Settings',
